$(function () {
    ...
    function worker() {
        callback = function(data, status, xhr) {
            ...
            dataBytes = data['pgmap']['data_bytes'];
            ...
            $("#utilization_info").html(fmtBytes(dataBytes) + " / " + fmtBytes(bytesUsed) + " / " + fmtBytes(bytesTotal) + " (" + percentUsed + "%)");
        ...
        ajaxCall(window.location.pathname + "api/cluster_status/", callback);
    };
    worker();
